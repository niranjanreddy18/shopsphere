...
export default function OrderSuccessPage() {
  ...
  const resolvedOrderId = id || searchParams.get("order_id");

  useEffect(() => {
    if (!resolvedOrderId) return;
    dispatch(fetchOrderDetail(resolvedOrderId));
  }, [dispatch, resolvedOrderId]);

  if (resolvedOrderId && (status === "loading" || !order)) return <FullPageSpinner />;
